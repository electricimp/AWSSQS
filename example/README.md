...
1. Click `Create New Queue` (or `Get Started Now` of you haven't used the service before)
1. Enter in `testQueue` in the `Queue Name` section
1. Note your AWS region
1. Select `Standard Queue`
1. Click `Quick-Create Queue`
1. Note the URL and ARN of the SQS queue you are using
...
1. On the `Edit Permissions` page do the following
    ...
    1. Use the ARN taken from the previous [step](#setting-up-a-queue-in-aws-sqs) for the `Amazon Resource Name (ARN)` field
